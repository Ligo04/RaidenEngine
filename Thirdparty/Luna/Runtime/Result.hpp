...
namespace Luna
{
    ...
    template <typename _Ty> struct [[nodiscard]] R
    {
        private:
            ErrCode            m_err_code;
        ...
        public:
            ...
            R(const R &rhs) : m_err_code(rhs.m_err_code)
            {
                if (valid())
                {
            ...
            R(R &&rhs) : m_err_code(rhs.m_err_code)
            {
                if (valid())
                {
            ...
            ErrCode errcode() const { return m_err_code; }
    };

    //! Specification of @ref R for void type.
    template <> struct [[nodiscard]] R<void>
    {
        private:
            ErrCode m_err_code;
    ...
    template <typename _Ty> bool succeeded(const R<_Ty> &r) { return r.valid(); }

    //! Tests whether the specified result is failed.
    //! @param[in] r The result to test.
    //! @return Returns `true` if the result is failed. Returns `false` otherwise.
    template <typename _Ty> bool failed(const R<_Ty> &r) { return !r.valid(); }

    //! Unwraps the real error code from the result.
    //! @details If the error code of this result object is `BasicError::error_object`, this function returns the error
    //! code stored in the error object of the current thread. If the error code of this object is not
    //! `BasicError::error_object`, this function returns the error code as is.
    ...
    template <typename _Ty> inline ErrCode unwrap_errcode(const R<_Ty> &obj) { return unwrap_errcode(obj.errcode()); }

    //! A special constant result object that represents one successful result.
    //! You can return `ok` instead of `RV()` to clearly represent one successful call for one function without return
    //! value.
    constexpr RV ok;
...
#define lupanic_if_failed(_res)                                                                                        \
    {                                                                                                                  \
        Luna::ErrCode _err = (_res).errcode();                                                                         \
        if (_err.code != 0)                                                                                            \
            Luna::assert_fail(Luna::explain(_err), luna_u8_string(__FILE__), (unsigned)(__LINE__));                    \
    }
...
#define lupanic_if_failed_msg(_res, _msg)                                                                              \
    {                                                                                                                  \
        Luna::ErrCode _err = (_res).errcode();                                                                         \
        if (_err.code != 0)                                                                                            \
            Luna::assert_fail(_msg, luna_u8_string(__FILE__), (unsigned)(__LINE__));                                   \
    }
...
//! @par Valid Usage
//! * Every function body can have only one try-catch block that encapsulates all expressions that may fail. If you need
//! multiple try-catch blocks, you can use dedicated functions to wrap each try-catch block, then call such functions in
