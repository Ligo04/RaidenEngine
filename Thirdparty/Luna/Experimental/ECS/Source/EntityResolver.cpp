...
namespace Luna
{
    namespace ECS
    {
        usize relocate_entity(World                             *world,
                              Cluster                           *src_cluster,
                              usize                              src_index,
                              Cluster                           *dst_cluster,
                              const HashMap<typeinfo_t, void *> &data)
        {
            ...
            while (src_component_index < src_components.size() && dst_component_index < dst_components.size())
            {
                ...
                if (src_component_type == dst_component_type)
                {
                    ...
                    void *dst_data
                        = (void *)((usize)dst_cluster->m_components[dst_component_index] + strip * dst_index);
                    void *src_data
                        = (iter2 != data.end())
                            ? iter2->second
        ...
        void EntityResolver::apply(World *world, entity_id_t entity)
        {
            ...
            if (record)
            {
                Cluster *dst_cluster = world->get_cluster({ m_component_types.data(), m_component_types.size() },
                                                          { m_tags.data(), m_tags.size() },
                                                          true);
        ...
        bool EntityResolver::add_component(typeinfo_t component)
        {
            ...
            auto  dst_components_end   = set_union(m_component_types.begin(),
        ...
        bool EntityResolver::remove_component(typeinfo_t component)
        {
            ...
            auto  dst_components_end   = set_difference(m_component_types.begin(),
                                                     m_component_types.end(),
                                                     &component,
                                                     &component + 1,
                                                     dst_components_begin);
        ...
        bool EntityResolver::add_tag(entity_id_t tag)
        {
            ...
            auto  dst_tags_end   = set_union(m_tags.begin(), m_tags.end(), &tag, &tag + 1, dst_tags_begin);
        ...
        bool EntityResolver::remove_tag(entity_id_t tag)
        {
            ...
            auto  dst_tags_end   = set_difference(m_tags.begin(), m_tags.end(), &tag, &tag + 1, dst_tags_begin);
            usize new_size       = (usize)(dst_tags_end - dst_tags_begin);
            if (new_size != m_tags.size())
            {
                m_tags.assign(Span<entity_id_t>(dst_tags_begin, new_size));
