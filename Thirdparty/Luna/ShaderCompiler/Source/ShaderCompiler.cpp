...
namespace Luna
{
    namespace ShaderCompiler
    {
                ...
                RV                      load_shader(const Path &path, IDxcBlob **ppIncludeSource)
                {
                    lutry
                    {
                        ...
                        if (m_included_files.find(path) == m_included_files.end())
                        {
                            lulet(
                                f,
                                open_file(path.encode().c_str(), FileOpenFlag::read, FileCreationMode::open_existing));
                            lulet(data, load_file_data(f));
                            m_compiler->m_dxc_utils->CreateBlob(data.data(),
                                                                (UINT32)data.size(),
                                                                CP_UTF8,
                                                                pEncoding.get_address_of());
        ...
        {
            if (params.shader_model.major < 6)
                return set_error(BasicError::not_supported(), "Shader model 5.1 and olders are not supported.");
            HRESULT hr;
            if (!m_dxc_compiler)
            {
            ...
            {
                case OptimizationLevel::none: argument_pointers.push_back(DXC_ARG_SKIP_OPTIMIZATIONS); break;
                case OptimizationLevel::speed: argument_pointers.push_back(DXC_ARG_OPTIMIZATION_LEVEL1); break;
                case OptimizationLevel::full: argument_pointers.push_back(DXC_ARG_OPTIMIZATION_LEVEL3); break;
            ...
            hr = m_dxc_compiler->Compile(&source,
                                         argument_pointers.data(),
                                         (UINT32)argument_pointers.size(),
                                         &include_handler,
                                         IID_PPV_ARGS(&result.m_dxc_result));
            if (FAILED(hr))
            {
                return BasicError::bad_platform_call();
            }
            // The shader compilation fail is not returned by error code for IDxcCompiler3::Compile, we need to handle
            // it explicitly.
            ...
            if (!(out_data && out_size))
            {
                ...
                {
                    return set_error(BasicError::bad_platform_call(),
        ...
        R<ShaderCompileResult> Compiler::spirv_compile(const ShaderCompileParameters &params,
                                                       SpirvOutputType                output_type)
        {
            ...
            lutry
            {
                ...
                if (output_type == SpirvOutputType::msl)
                {
                    ...
                    auto entry_point
                        = msl.get_entry_point(entry_point_and_stage.name, entry_point_and_stage.execution_model);
        ...
        R<ShaderCompileResult> Compiler::compile(const ShaderCompileParameters &params)
        {
            ...
            lutry
            {
                ...
                {
                    case TargetFormat::none: luset(r, compile_none(params)); break;
                    case TargetFormat::dxil:
                        {
                            lulet(dxc_result, dxc_compile(params, DxcTargetType::dxil));
                            r.data        = Blob((const byte_t *)dxc_result.m_dxc_blob->GetBufferPointer(),
                                          dxc_result.m_dxc_blob->GetBufferSize());
                    ...
                    case TargetFormat::msl: luset(r, spirv_compile(params, SpirvOutputType::msl)); break;
                    default: lupanic_msg("Unsupportted output format."); break;
                }
            }
            lucatchret;
            return r;
        ...
        LUNA_SHADER_COMPILER_API Ref<ICompiler> new_compiler() { return new_object<Compiler>(); }
