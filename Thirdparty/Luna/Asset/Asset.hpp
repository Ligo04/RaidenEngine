...
namespace Luna
{
    namespace Asset
    {
        ...
        struct asset_t
        {
                ...
                opaque_t handle;
                asset_t() : handle(nullptr) {}
                constexpr asset_t(opaque_t handle) : handle(handle) {}
                constexpr bool operator==(const asset_t &rhs) const { return handle == rhs.handle; }
                constexpr bool operator!=(const asset_t &rhs) const { return handle != rhs.handle; }
        ...
        struct AssetTypeDesc
        {
                //! The asset type name.
                Name name;
                //! The userdata object. The object will be kept by the asset system and provided to every callback
                //! function.
                ObjRef userdata;
                //! Called when one asset data object with data loaded from the specified file is being requested. The
                //! user should create a new asset data object and load asset data from the specified file to the
                //! object.
                //! @details This function can be `nullptr`. If this function is `nullptr`, this asset type does not
                //! support loading asset data from file, and such requests failed with @ref BasicError::not_supported.
                ...
                //! Called when one asset data object with default asset data is being requested. The user should create
                //! a new asset data object and load default asset data to the object.
                //! @details This function can be `nullptr`. If this function is `nullptr`, this asset type does not
                //! support loading default asset data, and such requests failed with @ref BasicError::not_supported.
                //! @param[in] userdata The userdata.
                //! @param[in] asset The asset handle of the asset being loaded.
                ...
                //!
                //! If this function fails, the new asset data object will not be set, that the existing asset data
                //! object is not changed.
                //!
                //! This function can be `nullptr`, in such case, the default callback function will be used, which
                //! simply does nothing and returns success directly.
                //! @param[in] userdata The userdata.
                //! @param[in] asset The asset handle of the asset to set new asset data object.
                //! @param[in] data The new asset data object to set.
                //! This can be `nullptr` if the user calls @ref set_asset_data with `data` equals to `nullptr`. In such
                //! case, this function behaves like unloading existing asset data object.
        ...
        //! function creates one new asset entry with the specified GUID and returns the handle to the new created asset
        //! entry.
        //! @details Asset handles created by @ref get_asset is in unregistered state ( @ref get_asset_state returns
        //! @ref AssetState::unregistered). The user should call @ref register_asset on the handle to register the asset
        //! before she can really use the asset. The user can also call @ref new_asset to create and register asset in
        //! one call.
        ...
        //! If `path` specifies one asset, the system loads the asset metadata by opening its meta file and reading from
        //! it. If `path` specifies one directory, the system loads assets metadata for all assets in the directory
        //! recursively, every asset metadata is loaded as if `load_assets_meta` is called for that particular asset.
        //! @param[in] allow_overwrite Specify the behavior when the specified asset already exists in the system.
        //! If `allow_overwrite` is `true`, the system will overwrite the asset metadata in the system using new asset
        //! metadata loaded from asset meta file; if `allow_overwrite` is `false`, the system discards the new asset
        //! metadata and does not change the asset metadata in the system if the specified asset already exists in the
        //! system. If `path` specifies one directory, this parameter is applied to all assets in that directory.
        ...
        //! @details This function only changes the asset's metadata in the system, it will not save the modified
        //! metadata to metadata file, it will also not move any asset file on VFS. The user should move asset files
        //! manually before calling this function, and call @ref save_asset_meta manually after this function.
        //!
        //! The user can get files that should be moved by @ref get_asset_files. The user can also call @ref move_asset
        //! to move asset files, change asset path and save asset metadata to file in one call, which is preferred if
        //! the user does not need to perform custom copy tasks.
        //! @param[in] asset The asset handle of the asset to operate.
        ...
        //! Gets the asset data object.
        //! @param[in] asset The asset handle of the asset to query.
        //! @param[in] block_until_loaded If the asset is not loaded and this is `true`, this call blocks the current
        //! thread until the asset data is loaded, then returns the loaded asset data. Note that this may cause
        //! application freeze if improperly used.
        //! @return Returns the asset data, or `nullptr` if the asset data is not loaded or failed to load.
        template <typename _Ty> Ref<_Ty> get_asset_data(asset_t asset) { return Ref<_Ty>(get_asset_data(asset)); }

        //! Sets the asset data object.
        //! @param[in] asset The asset handle of the asset to operate.
        //! @param[in] data The asset data to set. If this is `nullptr`, the asset data will be cleared, and the asset
        //! will be in
        ...
        //! @param[in] asset The asset handle of the asset to operate.
        //! @param[in] force_reload If this is `true`, this function always loads the asset data from file even if the
        //! asset is in loaded state, and the existing asset data object will be replaced with new asset data object. If
        //! this is `false`, this function returns directly if the asset is already in loaded state, and the existing
        //! asset data object will not be changed.
        ...
        //! If this is `false`, this function returns directly if the asset is already in loaded state, and the existing
        //! asset data object will not be changed.
