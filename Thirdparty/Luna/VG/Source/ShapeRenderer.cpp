...
namespace Luna
{
    namespace VG
    {
        ...
        RV                                  init_render_resources()
        {
            ...
            lutry
            {
                ...
                }
                {
                    DescriptorSetLayoutBinding bindings[]
                        = { DescriptorSetLayoutBinding::uniform_buffer_view(0, 1, ShaderVisibilityFlag::vertex),
                            ...
                            DescriptorSetLayoutBinding::read_texture_view(TextureViewType::tex2d,
                                                                          2,
                                                                          1,
                                                                          ShaderVisibilityFlag::pixel),
                ...
                }
                {
                    ...
                    PipelineLayoutDesc    desc({ &dl, 1 }, PipelineLayoutFlag::allow_input_assembler_input_layout);
                    luset(g_fill_playout, dev->new_pipeline_layout(desc));
                }
                {
                    TextureDesc desc = TextureDesc::tex2d(Format::rgba8_unorm,
                                                          TextureUsageFlag::read_texture | TextureUsageFlag::copy_dest,
                                                          1,
                                                          1);
                    ...
                    u32 data = 0xFFFFFFFF;
                    {
                        ...
                        luexp(copy_resource_data(upload_cmdbuf,
                                                 { CopyResourceData::write_texture(g_white_tex,
                                                                                   SubresourceIndex(0, 0),
                                                                                   0,
                                                                                   0,
                                                                                   0,
                                                                                   &data,
        ...
        RV FillShapeRenderer::create_pso(RHI::Format rt_format)
        {
            ...
            lutry
            {
                ...
                desc.rasterizer_state
                    = RasterizerDesc(FillMode::solid, CullMode::back, false, false, false, false, false);
        ...
        RV FillShapeRenderer::init(RHI::ITexture *render_target) { return set_render_target(render_target); }
        ...
        RV FillShapeRenderer::render(RHI::ICommandBuffer      *cmdbuf,
                                     ...
                                     Float4x4U                *transform_matrix)
        {
            ...
            lutry
            {
                u32 cb_element_size = (u32)align_upper(
                    sizeof(Float4x4U),
                    dev->check_feature(DeviceFeature::uniform_buffer_data_alignment).uniform_buffer_data_alignment);
                usize num_draw_calls = draw_calls.size();
                u64   cb_size        = cb_element_size * num_draw_calls;
                // Build constant buffer.
                if (num_draw_calls > m_cbs_capacity)
                {
                    luset(m_cbs_resource,
                          dev->new_buffer(MemoryType::upload, BufferDesc(BufferUsageFlag::uniform_buffer, cb_size)));
                ...
                for (usize i = 0; i < num_draw_calls; ++i)
                {
                    ...
                    transform            = mul(transform,
                                    AffineMatrix::make_translation(
                                        Float3(draw_calls[i].origin_point.x, draw_calls[i].origin_point.y, 0.0f)));
                    if (transform_matrix)
                    {
                    ...
                    {
                        Float4x4 mat = ProjectionMatrix::make_orthographic_off_center(0.0f,
                                                                                      (f32)m_screen_width,
                                                                                      0.0f,
                                                                                      (f32)m_screen_height,
                                                                                      0.0f,
                ...
                for (usize i = 0; i < num_draw_calls; ++i)
                {
                    ...
                    luexp(ds->update_descriptors(
                        { WriteDescriptorSet::uniform_buffer_view(
                              0,
                              BufferViewDesc::uniform_buffer(m_cbs_resource, i * cb_element_size)),
                          WriteDescriptorSet::read_buffer_view(
                              1,
                              BufferViewDesc::structured_buffer(dc.shape_buffer, 0, num_points, 4)),
                          WriteDescriptorSet::read_texture_view(
                              2,
                              TextureViewDesc::tex2d(dc.texture ? dc.texture : g_white_tex)),
                          WriteDescriptorSet::sampler(3, dc.sampler) }));
                }
                // Build command buffer.
                Vector<TextureBarrier> barriers;
                barriers.push_back({ m_render_target,
                                     SubresourceIndex(0, 0),
                                     TextureStateFlag::automatic,
                                     TextureStateFlag::color_attachment_write,
                                     ResourceBarrierFlag::discard_content });
                barriers.push_back({ g_white_tex,
                                     TEXTURE_BARRIER_ALL_SUBRESOURCES,
                                     TextureStateFlag::automatic,
                                     TextureStateFlag::shader_read_ps,
                                     ResourceBarrierFlag::none });
                for (usize i = 0; i < num_draw_calls; ++i)
                {
                    if (draw_calls[i].texture)
                    {
                        barriers.push_back({ draw_calls[i].texture,
                                             TEXTURE_BARRIER_ALL_SUBRESOURCES,
                                             TextureStateFlag::automatic,
                                             TextureStateFlag::shader_read_ps,
                                             ResourceBarrierFlag::none });
                ...
                desc.color_attachments[0]
                    = ColorAttachment(m_render_target, LoadOp::clear, StoreOp::store, Float4U{ 0.0f });
        ...
        LUNA_VG_API R<Ref<IShapeRenderer>> new_fill_shape_renderer(RHI::ITexture *render_target)
        {
            ...
            lutry { luexp(renderer->init(render_target)); }
            lucatchret;
            ret = renderer;
            return ret;
        }
