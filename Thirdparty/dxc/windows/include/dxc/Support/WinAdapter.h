...
#define MAKE_HRESULT(severity, facility, code)                                                                         \
    ((HRESULT)(((unsigned long)(severity) << 31) | ((unsigned long)(facility) << 16) | ((unsigned long)(code))))
...
inline bool IsEqualIID(REFIID riid1, REFIID riid2) { return IsEqualGUID(riid1, riid2); }

inline bool IsEqualCLSID(REFCLSID rclsid1, REFCLSID rclsid2) { return IsEqualGUID(rclsid1, rclsid2); }
...
constexpr uint8_t nybble_from_hex(char c)
{
    return ((c >= '0' && c <= '9')
                ? (c - '0')
                : ((c >= 'a' && c <= 'f') ? (c - 'a' + 10)
                                          : ((c >= 'A' && c <= 'F') ? (c - 'A' + 10) : /* Should be an error */ -1)));
}

constexpr uint8_t byte_from_hex(char c1, char c2) { return nybble_from_hex(c1) << 4 | nybble_from_hex(c2); }

constexpr uint8_t byte_from_hexstr(const char str[2]) { return nybble_from_hex(str[0]) << 4 | nybble_from_hex(str[1]); }

constexpr GUID    guid_from_string(const char str[37])
{
    return GUID{
        static_cast<uint32_t>(byte_from_hexstr(str)) << 24 | static_cast<uint32_t>(byte_from_hexstr(str + 2)) << 16
            | static_cast<uint32_t>(byte_from_hexstr(str + 4)) << 8 | byte_from_hexstr(str + 6),
        static_cast<uint16_t>(static_cast<uint16_t>(byte_from_hexstr(str + 9)) << 8 | byte_from_hexstr(str + 11)),
        static_cast<uint16_t>(static_cast<uint16_t>(byte_from_hexstr(str + 14)) << 8 | byte_from_hexstr(str + 16)),
        { byte_from_hexstr(str + 19),
                                                                                                         byte_from_hexstr(str + 21),
                                                                                                         byte_from_hexstr(str + 24),
                                                                                                         byte_from_hexstr(str + 26),
                                                                                                         byte_from_hexstr(str + 28),
                                                                                                         byte_from_hexstr(str + 30),
                                                                                                         byte_from_hexstr(str + 32),
                                                                                                         byte_from_hexstr(str + 34) }
    };
}
...
template <class T, class Allocator = CAllocator> class CHeapPtrBase
{
    ...
    public:
        ...
        {
            T *pNew;
            pNew = static_cast<T *>(Allocator::Reallocate(m_pData, nBytes * sizeof(char)));
            if (pNew == NULL)
                return false;
            m_pData = pNew;
...
template <typename T, class Allocator = CAllocator> class CHeapPtr: public CHeapPtrBase<T, Allocator>
{
    public:
        CHeapPtr() throw() {}
    ...
    public:
        CW2AEX(LPCWSTR psz, UINT nCodePage = CP_UTF8)
        {
            ...
            if (locale == nullptr)
            {
                ...
                assert(false
                       && "CW2AEX implementation for Linux only handles "
    ...
    public:
        CA2WEX(LPCSTR psz, UINT nCodePage = CP_UTF8)
        {
            ...
            if (locale == nullptr)
            {
                // Current Implementation only supports CP_UTF8, and CP_ACP
                assert(false
                       && "CA2WEX implementation for Linux only handles "
                          "UTF8 and ACP code pages");
                return;
            }
